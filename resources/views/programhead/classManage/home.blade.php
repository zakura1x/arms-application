...
<body class="bg-[#EFF4F6] font-poppins antialiased text-black">
                ...
                <div class="p-8 bg-[#EFF4F6] shadow-lg rounded-lg border border-gray-200 h-full">
                    <header class="flex items-center bg-[#D0D9D3] text-black p-2">
                        <div class="px-4 flex items-center mr-96">
                            <a href="{{ route('ph.class-list') }}">
                                <svg width="30" height="30" viewBox="0 0 30 30" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M14.7402 28.4805C22.3288 28.4805 28.4805 22.3288 28.4805 14.7402C28.4805 7.15171 22.3288 1 14.7402 1C7.15171 1 1 7.15171 1 14.7402C1 22.3288 7.15171 28.4805 14.7402 28.4805Z"
                                        stroke="black" stroke-width="2" stroke-linecap="round"
                                        stroke-linejoin="round" />
                                    <path d="M20.117 14.7402H9.96094" stroke="black" stroke-width="2"
                                        stroke-linecap="round" stroke-linejoin="round" />
                                    <path d="M12.948 11.7532L9.96143 14.7403L12.948 17.7274" stroke="black"
                                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                            </a>
                            <h2 class="ml-4 font-semibold">{{ $class->name }}</h2>
                        </div>
                        ...
                        <div class="tabs">
                            <ul class="flex border-b">
                                <li class="mr-1">
                                    <a class="bg-white inline-block py-2 px-4 font-semibold text-gray-800 border-l border-t border-r rounded-t active"
                                        href="#tab1">STUDENT</a>
                                </li>
                                <li class="mr-1">
                                    <a class="bg-white inline-block py-2 px-4 text-gray-800 hover:text-gray-600 font-semibold border-l border-t border-r rounded-t"
                                        href="#tab2">MODULE</a>
                                </li>
                                <li class="mr-1">
                                    <a class="bg-white inline-block py-2 px-4 text-gray-800 hover:text-gray-600 font-semibold border-l border-t border-r rounded-t"
                                        href="#tab3">ASSESSMENT</a>
                                </li>
                            </ul>
                        </div>
                    </header>
                    ...
                    <div class="tab-contents mt-4">
                        <div id="tab1" class="tab-content bg-white border border-gray-200 rounded-lg active">
                            @include('programhead.classmanage.students')
                        </div>
                        ...
                        <div id="tab3" class="tab-content bg-white border border-gray-200 rounded-lg hidden">
                            @include('programhead.classmanage.assessment')
                        </div>
                    </div>
    ...
    {{-- where should be the file placement of the js --}}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const editAssessment = document.querySelectorAll('#editAssessment');
            ...
            createAssessment.addEventListener('click', function() {
                window.location.href = '{{ route('ph.assessment-create', ['classID' => $class->id]) }}';
            });

            editAssessment.forEach((button) => {
                button.addEventListener('click', function() {
                    const assessmentId = button.getAttribute('data-assessment-id');
                    window.location.href = `/program-head/assessment-edit/${assessmentId}`;
                });
